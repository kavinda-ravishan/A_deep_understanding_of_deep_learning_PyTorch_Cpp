...
#pragma region Overfittingand_cross_validation

void Overfittingand_cross_validation::iris_dataset_dataloader_barch_train()
{
	...
	torch::Tensor pred_train = torch::argmax(ANNclassify->forward(X_train_t), 1);
	torch::Tensor pred_test = torch::argmax(ANNclassify->forward(X_test_t), 1);

	int train_numPreds = pred_train.size(0);
	int num_correct_preds_train = 0;
	for (int i = 0; i < train_numPreds; i++)
	{
		if (pred_train[i].item<int>() == y_train_t[i].item<int>()) num_correct_preds_train++;
	}

	int test_numPreds = pred_test.size(0);
	int num_correct_preds_test = 0;
	for (int i = 0; i < test_numPreds; i++)
	{
		if (pred_test[i].item<int>() == y_test_t[i].item<int>()) num_correct_preds_test++;
	}

	std::cout << "Training Accuracy : " << (num_correct_preds_train * 100) / float(train_numPreds) << "%" << std::endl;
	std::cout << "Testing Accuracy : " << (num_correct_preds_test * 100) / float(test_numPreds) << "%" << std::endl;
}

std::pair<int, int> Overfittingand_cross_validation::train_test_split(
...
std::vector<std::string> Overfittingand_cross_validation::split(const std::string& s, char delimiter) {
...
int Overfittingand_cross_validation::read_csv(
...
void Overfittingand_cross_validation::ALLCalls()
{
